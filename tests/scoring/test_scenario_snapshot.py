...
def test_a_snapshot_of_an_including_scenario_stands_alone(tmp_path: Path) -> None:
    ...
    _snapshot_scenario(_BENCHMARK_DIR / "t2_256_flat_1001_realtime.conf", destination)
...
def test_a_snapshot_yields_the_same_settings_as_the_source(tmp_path: Path) -> None:
    ...
    destination = tmp_path / "nttd_scenario.conf"
    _snapshot_scenario(_BENCHMARK_DIR / "t3_512_hilly_2001_realtime.conf", destination)

    source_settings = scenario_to_settings(load(_BENCHMARK_DIR / "t3_512_hilly_2001_realtime.conf"), strict=True)
